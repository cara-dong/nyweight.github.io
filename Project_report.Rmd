...
Our investigation in class regarding NYC Airbnb prices and some projects past students have done about NYC inspired us to pursue a journey in exploring the student weight status and other factors in NY state.

# Initial Questions

We were initially trying to resolve question the question "Does food price affect students' weight status?", but as we dived deeper into the project, our questions expand into the form of "What factors contribute to the percentage of overweight students in NY state?", "Are there any factors that are more influential?" and "Can we predict weight status from these factors?".
...
* We renamed `district` to `location_code` to make this dataset easier to join with the other on the common-named column.
...
Following this, we generated scatter plots to visualize the county-level average distribution of racial groups and the corresponding average percentage of overweight individuals to guide out variable selection in linear models.
...
After checking that there is no highly correlated pairs in the predictors, we fitted a model that analyzed the relationship between student demographics and overweight percentages at the district level.
```{r message=FALSE, warning=FALSE, echo=FALSE}
...
Contributors: Manye Dong md4270@cumc.columbia.edu, Haotian Tang ht2630@cumc.columbia.edu, Jingyi Xu jx2562@cumc.columbia.edu, Longyi Zhao lz2949@cumc.columbia.edu